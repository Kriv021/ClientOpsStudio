...
QString extractSingleLineField(const QString& text, const QString& label) {
    const QRegularExpression re(QString(R"(^%1:\s*(.+)$)").arg(QRegularExpression::escape(label)),
                                QRegularExpression::MultilineOption);
    const auto match = re.match(text);
    return match.hasMatch() ? match.captured(1).trimmed() : QString();
}

QString extractBlockField(const QString& text, const QString& label, const QStringList& stopLabels) {
    const int begin = text.indexOf(label + ":");
    if (begin < 0) {
        return QString();
    }

    int start = begin + label.size() + 1;
    if (start < text.size() && text.at(start) == '\n') {
        ++start;
    }
    while (start < text.size() && text.at(start).isSpace() && text.at(start) != '\n') {
        ++start;
    }

    int end = text.size();
    for (const auto& stop : stopLabels) {
...
bool isKnownCandidateName(const DiagnosisResult& diagnosis, const QString& name) {
    for (const auto& candidate : diagnosis.candidates) {
...
void MainWindow::wireSignals() {
    ...
    connect(&aiAdvisor_, &AiAdvisor::adviceReady, this, [this](const QString& advice) {
        ...
        lastDiagnosis_.aiReviewAction = extractSingleLineField(advice, "复核动作");
        const QString reviewedCause = extractSingleLineField(advice, "AI主因");
        if (isKnownCandidateName(lastDiagnosis_, reviewedCause)) {
            lastDiagnosis_.aiReviewedCause = reviewedCause;
            lastDiagnosis_.finalConclusion = reviewedCause;
        } else {
            lastDiagnosis_.aiReviewedCause.clear();
            lastDiagnosis_.finalConclusion = lastDiagnosis_.rulePrimaryCause;
        }
        lastDiagnosis_.aiReviewNote = extractBlockField(
            advice,
            "AI说明",
            {"验证步骤", "修复建议"});
        renderDiagnosis();
    ...
    connect(&aiAdvisor_, &AiAdvisor::adviceError, this, [this](const QString& error) {
        ...
        lastDiagnosis_.aiReviewAction = "insufficient";
        lastDiagnosis_.aiReviewedCause.clear();
        lastDiagnosis_.aiReviewNote = error;
        lastDiagnosis_.finalConclusion = lastDiagnosis_.rulePrimaryCause;
        renderDiagnosis();
...
void MainWindow::runDiagnosis() {
    ...
    scopedSignals_ = signalExtractor_.extract(scopedLogs_);

    HttpResponseData basis = lastResponse_;
    if (!hasRequestTime) {
        basis = HttpResponseData();
    }
    // 规则诊断只依赖结构化信号，避免直接对原始日志文案做 if-else。
    lastDiagnosis_ = diagnosisEngine_.diagnose(basis, scopedSignals_, corr.mode, corr.evidence);
    lastAiAdvice_.clear();
    renderDiagnosis();
...
QString MainWindow::buildAiPrompt() const {
    ...
    // AI 只复核规则已收敛出来的候选和证据，不直接吃整段原始日志，
    // 这样能降低噪声、控制 token，并保证可解释性。
    s << "你是受约束的故障诊断复核器，只能在给定候选根因集合内进行判断，不允许发明新的根因类别。\n";
    s << "如果证据不足，可以选择“证据不足/未知”或降低置信度。\n\n";

    s << "Case 信息:\n";
    s << "- 问题: " << caseProblemEdit_->text() << "\n";
    s << "- 接口: " << caseTargetEdit_->text() << "\n";
    s << "- 请求方式: " << lastRequest_.method << "\n";
    s << "- 请求地址: " << lastRequest_.url << "\n\n";
    ...
    s << "规则侧主因:\n";
    s << "- " << lastDiagnosis_.rulePrimaryCause << "\n";
    s << "- 摘要: " << lastDiagnosis_.summary << "\n\n";

    s << "候选根因 Top3:\n";
    for (const auto& candidate : lastDiagnosis_.candidates) {
        s << "- " << candidate.name << " | score=" << candidate.score << " | " << candidate.summary << "\n";
    }
    s << "\n";

    s << "关键证据:\n";
    int n = 0;
    for (const auto& item : lastDiagnosis_.evidenceItems) {
        s << "- [" << signalTypeName(item.signalType) << "] "
          << item.module << " "
    ...
    s << "请严格按以下格式输出：\n";
    s << "复核动作: agree|reorder|downgrade|insufficient\n";
    s << "AI主因: <必须从候选根因 Top3 中选择一个>\n";
    s << "AI说明:\n";
    s << "<说明为什么同意/质疑规则主因，指出哪个候选只是伴随症状>\n";
    s << "验证步骤:\n";
    s << "1. ...\n";
    s << "2. ...\n";
    s << "修复建议:\n";
...
void MainWindow::renderDiagnosis() {
    ...
    s << "规则主因: " << (lastDiagnosis_.rulePrimaryCause.isEmpty() ? "-" : lastDiagnosis_.rulePrimaryCause) << "\n";
    s << "最终结论: " << (lastDiagnosis_.finalConclusion.isEmpty() ? lastDiagnosis_.rulePrimaryCause : lastDiagnosis_.finalConclusion) << "\n\n";
    ...
    if (!lastDiagnosis_.candidates.isEmpty()) {
        s << "候选根因 Top" << lastDiagnosis_.candidates.size() << ":\n";
        int rank = 1;
        for (const auto& candidate : lastDiagnosis_.candidates) {
            s << rank++ << ". " << candidate.name << "（score=" << candidate.score << "）\n";
            s << "   " << candidate.summary << "\n";
            if (!candidate.primaryEvidence.isEmpty()) {
                s << "   主证据: " << candidate.primaryEvidence.first().snippet << "\n";
            }
            s << "   下一步: " << candidate.nextSteps << "\n\n";
        }
    }

    if (!lastDiagnosis_.evidenceItems.isEmpty()) {
        s << "关键证据:\n";
        int evidenceCount = 0;
        for (const auto& item : lastDiagnosis_.evidenceItems) {
            s << "- [" << signalTypeName(item.signalType) << "] " << item.snippet << "\n";
            if (++evidenceCount >= 6) {
                break;
            }
        }
    ...
    if (!lastDiagnosis_.aiReviewAction.isEmpty()) {
        s << "AI 复核:\n";
        s << "- 动作: " << lastDiagnosis_.aiReviewAction << "\n";
        s << "- AI 主因: " << (lastDiagnosis_.aiReviewedCause.isEmpty() ? "未改判" : lastDiagnosis_.aiReviewedCause) << "\n";
        if (!lastDiagnosis_.aiReviewNote.trimmed().isEmpty()) {
            s << "- 说明: " << lastDiagnosis_.aiReviewNote << "\n";
        }
        s << "\n";
    }
